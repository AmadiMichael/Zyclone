...
- `ETHZyclone.huff`: Inherits `Zyclone.huff` abstract huff file and implements `processDeposit` and `processWithdraw` to make it work as an ETH mixer. Also holds the constructor code.

Each relevant external function is heavily documented with calldata layout well defined.
